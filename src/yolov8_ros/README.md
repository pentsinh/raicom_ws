...
* **`imgsz`** 
    ...
    YOLO inference input size. Smaller values reduce GPU load and memory usage.

* **`max_det`** 

    Maximum number of detections kept per frame.

* **`half`** 

    Enable FP16 inference on GPU. This reduces VRAM usage and can improve throughput.
